...
import sqlite3, os
import hashlib # необязательно, но желательно
    ...
    if request.method == "POST":
        ...
        with sqlite3.connect('database.db') as conn:
            try:
                ...
                conn.commit()
                msg = "Пользователь успешно обработан"
            except:
                msg = "ОШИБКА"
                conn.rollback()
        conn.close()
...
def editProfile():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        cur.execute("SELECT userId, email, firstName, lastName,"
                    ...
                    " phone FROM users WHERE email = ?", (session['email'], ))
...
def productDescription():
    ...
    with sqlite3.connect('database.db') as conn:
        cur = conn.cursor()
        cur.execute('SELECT productId, name, price, description, image,'
                    ' stock FROM products WHERE productId = ?', (productId, ))
...
def cart():
    ...
    with sqlite3.connect('database.db') as conn:
        ...
        cur.execute("SELECT products.productId, products.name, products.price,"
                    ...
                    " kart WHERE products.productId = kart.productId AND kart.userId = ?",
                    (userId, ))
